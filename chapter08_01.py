...
print(list(filter(conv_pos, [1, -3, 2, 0 , 4, -5])))
print(list(filter(lambda x: abs(x)> 2, [1, -3, 2, 0 , 4, -5] )))
...
print(min([1,2,3]))
print(min('python study')) #공백이 출력됨

# map : 반복가능한 객체 요소를 지정한 함수 실행 후 추출
def conv_abs(x):
    return abs(x)

print(list(map(conv_abs, [-1, 3, 0, -9])))
print(list(map(lambda x:abs(x),[-1, 3, 0, -9])))

# pow : 제곱값 반환
print(pow(2,10))

# range : 반복가능한 객체(Iterable) 반환 
# 연속된 숫자(정수)를 만들어주는 range
print(range(1,10,2))
...
print(a)
print(sorted(['p','y','t','h','o','n'])) #오름차순으로 정렬
